    ...
    FactorAnimator.Target
{
    ...
    public interface ChatActivityEnterViewDelegate {
        ...
        default void onMessageSend(CharSequence message, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars, Integer autoDeleteDelay) {
            onMessageSend(message, notify, scheduleDate, scheduleRepeatPeriod, payStars);
        }
...
//        }
    ...
    protected boolean sendMessageInternal(boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars, boolean allowConfirm, Integer autoDeleteDelay) {
        final Runnable send = () -> {
            ...
            if (allowConfirm && showConfirmAlert(() -> {
                sendMessageInternal(notify, scheduleDate, scheduleRepeatPeriod, payStars, false, autoDeleteDelay);
            })) {
                return;
            }
            ...
            if (videoToSendMessageObject != null) {
                ...
                SendMessagesHelper.SendMessageParams params = SendMessagesHelper.SendMessageParams.of(audioToSend, null, audioToSendPath, dialog_id, replyingMessageObject, getThreadMessage(), null, null, null, null, notify, scheduleDate, 0, voiceOnce ? 0x7FFFFFFF : 0, null, null, false).addAutoDeleteDelay(autoDeleteDelay);
                params.quick_reply_shortcut = parentFragment != null ? parentFragment.quickReplyShortcut : null;
                params.quick_reply_shortcut_id = parentFragment != null ? parentFragment.getQuickReplyId() : 0;
                params.effect_id = effectId;
                ...
                SendMessagesHelper.getInstance(currentAccount).sendMessage(params);
                if (delegate != null) {
                    delegate.onMessageSend(null, notify, scheduleDate, scheduleRepeatPeriod, payStars, autoDeleteDelay);
                }
                hideRecordedAudioPanel(true);
                checkSendButton(true);
                AndroidUtilities.runOnUIThread(() -> {
            ...
            if (processSendingText(message, notify, scheduleDate, scheduleRepeatPeriod, payStars, autoDeleteDelay)) {
                if (delegate.hasForwardingMessages() || (scheduleDate != 0 && !isInScheduleMode()) || isInScheduleMode()) {
                    if (messageEditText != null) {
                        messageEditText.setText("");
                    }
                    if (delegate != null) {
                        delegate.onMessageSend(message, notify, scheduleDate, scheduleRepeatPeriod, payStars, autoDeleteDelay);
                    }
                } else {
                    messageTransitionIsRunning = false;
                    AndroidUtilities.runOnUIThread(moveToSendStateRunnable = () -> {
    ...
    public boolean processSendingText(CharSequence text, boolean notify, int scheduleDate, int scheduleRepeatPeriod, long payStars, Integer autoDeleteDelay) {
        if (replyingQuote != null && parentFragment != null && replyingQuote.outdated) {
            parentFragment.showQuoteMessageUpdate();
            return false;
        }
        ...
        if (text.length() != 0) {
            ...
            do {
                ...
                SendMessagesHelper.SendMessageParams params = SendMessagesHelper.SendMessageParams.of(message[0].toString(), dialog_id, replyingMessageObject, replyToTopMsg, messageWebPage, messageWebPageSearch, entities, null, null, notify, scheduleDate, scheduleRepeatPeriod, sendAnimationData, updateStickersOrder).addAutoDeleteDelay(autoDeleteDelay);
                params.quick_reply_shortcut = parentFragment != null ? parentFragment.quickReplyShortcut : null;
                params.quick_reply_shortcut_id = parentFragment != null ? parentFragment.getQuickReplyId() : 0;
                params.effect_id = effectId;
                params.payStars = payStars;
                ...
                if (parentFragment != null && parentFragment.isEncryptedGroup()) {
                    ...
                    if (encryptedGroupId != null) {
                        randomId = SendMessagesHelper.getInstance(currentAccount).getNextRandomId();
                    }
...
//        }
    ...
    public void updateSendAsButton(boolean forceHide, boolean animated) {
        ...
        if (isLiveComment) {
            ...
            defPeer = full != null && !FakePasscodeUtils.isHidePeer(full.default_send_as, currentAccount) ? full.default_send_as : null;
        }
        ...
        boolean isVisible = !forceHide && defPeer != null && (delegate.getSendAsPeers() == null || FakePasscodeUtils.filterSendAsPeers(delegate.getSendAsPeers().peers, currentAccount).size() > 1) &&
