...
public class ChatActivityEnterView extends BlurredFrameLayout implements NotificationCenter.NotificationCenterDelegate, SizeNotifierFrameLayout.SizeNotifierFrameLayoutDelegate, StickersAlert.StickersAlertDelegate, SuggestEmojiView.AnchorViewDelegate {
    ...
    public interface ChatActivityEnterViewDelegate {
        ...
        default void onMessageSend(CharSequence message, boolean notify, int scheduleDate, long payStars, Integer autoDeleteDelay) {
            onMessageSend(message, notify, scheduleDate, payStars);
        }
    ...
    protected boolean sendMessageInternal(boolean notify, int scheduleDate, long payStars, boolean allowConfirm, Integer autoDeleteDelay) {
        final Runnable send = () -> {
            ...
            if (allowConfirm && showConfirmAlert(() -> {
                sendMessageInternal(notify, scheduleDate, payStars, false, autoDeleteDelay);
            })) {
                return;
            }
            ...
            if (videoToSendMessageObject != null) {
                ...
                SendMessagesHelper.getInstance(currentAccount).sendMessage(params);
                if (delegate != null) {
                    delegate.onMessageSend(null, notify, scheduleDate, payStars, autoDeleteDelay);
                }
                hideRecordedAudioPanel(true);
                checkSendButton(true);
                AndroidUtilities.runOnUIThread(() -> {
            ...
            if (processSendingText(message, notify, scheduleDate, payStars, autoDeleteDelay)) {
                if (delegate.hasForwardingMessages() || (scheduleDate != 0 && !isInScheduleMode()) || isInScheduleMode()) {
                    if (messageEditText != null) {
                        messageEditText.setText("");
                    }
                    if (delegate != null) {
                        delegate.onMessageSend(message, notify, scheduleDate, payStars, autoDeleteDelay);
                    }
                } else {
                    messageTransitionIsRunning = false;
                    AndroidUtilities.runOnUIThread(moveToSendStateRunnable = () -> {
